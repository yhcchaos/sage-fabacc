...
then
    ...
    echo "you gave $# inputs, but we need 19 inputs!"
...
path=/home/yhchaos/test/ccBench/pantheon-modified/third_party/sage/sage_rl
log=${prefix}-sage-$num_flows-wired$bw-x1-35-$latency-$qsize-$loss-$run_id
basetimestamp_fld=$data_dir/sage_mm_datalink_run$run_id.log_init_timestamp
echo "sdfasdsssssssssssssssssssssssssssssssssssssssssss"
echo "base timestamp folder: $basetimestamp_fld"

echo "will be done in $t-second/$num_steps-Step ..."
sudo -u `logname` $path/server.sh $port $period $first_time $scheme "$path/rl_module" $actor_id $dl $up $latency $log $t $loss $qsize $num_steps $basetimestamp_fld $bw $bw2 $trace_period $save $num_flows
